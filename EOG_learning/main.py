...
if __name__ == "__main__":
    EEG_EOG_Cat_real = np.load(r'data\EEG_EOG_Cat.npy')
    EEG_EOG_Cat_classification = np.load(r'data\EEG_EOG_Cat_real.npy')
    Errp_all_channels = np.load(r'data\Errp_all_channels.npy')
    ...
    chart = Chart(EEG_EOG_Cat[:][:],min = np.min(EEG_EOG_Cat), max = np.max(EEG_EOG_Cat),\
        real_data=EEG_EOG_Cat_real, EEG_EOG_Cat_classification=EEG_EOG_Cat_classification, Errp_data = Errp_all_channels)
    chart.legend().hide()
